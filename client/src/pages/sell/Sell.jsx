...
export const Sell = () => {
  ...
  useEffect(() => {
    const fetchSellProperties = async () => {
      ...
      setError('')
      try {
        const response = await Axios.get(SummaryApi.getProperties.url, { params: { type: 'sell' } })
        setProperties(response?.data?.properties || [])
      } catch (err) {
        setError(err?.response?.data?.message || 'Unable to load sell properties.')
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 font-sans bg-white">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div className="text-[15px] text-[#333]">
          {loading ? 'Loading properties...' : `Showing ${sortedProperties.length} sell properties`}
        </div>
          ...
          <div key={property._id} className="group cursor-pointer flex flex-col">
            ...
            <div className="flex flex-col">
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-[15px] text-[#888]">{property.status || 'For Sale'}</span>
